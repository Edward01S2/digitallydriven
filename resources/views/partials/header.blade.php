<header class="relative z-40">
  <nav x-data="{ open: false, country: false }" class="fixed z-40 w-full bg-white shadow-md nav-container">
        ...
        <div class="md:bg-white md:absolute md:top-0 md:right-0 md:shadow md:px-10 md:py-6 md:pb-10 md:mr-6">
          <div>
            @foreach ($navi as $item)
              ...
              @else
                <div class="md:pb-2">
                  <a @click="open = false" class="inline-block text-lg py-3 nav-link md:pb-1 font-aktiv-ex font-semibold tracking-wider text-c-tan-100 transition duration-300 ease-in-out border-transparent border-b-2 hover:border-c-orange-100 focus:outline-none md:text-base {{ $item->classes ?? '' }} {{ $item->active ? 'active' : '' }}" href="{{ $item->url }}" target="{{ $item->target }}">
                    {{ $item->label }}  
                  </a>
              </div>
              @endif
